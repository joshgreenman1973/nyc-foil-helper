...
| [OpenRecords FOIL request log](https://data.cityofnewyork.us/d/kegn-anvq) (`kegn-anvq`), via the [FOIL Tracker](https://joshgreenman1973.github.io/experiments/nyc-foil-tracker/) | Agency track record | Every request filed through OpenRecords since 2006. This tool does not process it: the Tracker does, publishing `data/performance.json`, which this page fetches at runtime and reads one agency's row from. |

## The agency track record

Once a request is routed, the page shows what that agency actually does: median wait on what it
closed in the last year, requests sitting past due, the share that met the date the agency set
itself, and how far out it typically sets that date. Three things to know about those numbers.

- **They are computed elsewhere.** The [FOIL Tracker](https://joshgreenman1973.github.io/experiments/nyc-foil-tracker/)
  owns the whole-log analysis, and its
  [methodology](https://joshgreenman1973.github.io/experiments/nyc-foil-tracker/methodology.html)
  defines every figure. Keeping one copy means the two pages cannot drift apart.
- **"Met own deadline" is not a legal standard.** FOIL requires acknowledgment within five business
  days and then a date certain, which an agency may extend. The percentage measures a promise the
  agency wrote, so an agency that promises a year out can score highly while being slow — which is
  why the page also shows how far out it sets that date.
- **Six agencies have no record at all.** NYCHA, the Comptroller, the Board of Elections, EDC, the
  School Construction Authority and DOHMH do not take FOIL through OpenRecords, so nothing about
  them appears in the log. The page says so rather than showing an empty scorecard.
